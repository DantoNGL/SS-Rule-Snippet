...
$.token = 'f85997883c9e5981248a2cd5ff40d97018df651e';

// STEP 1: 前往 https://aqicn.org/data-platform/token/ 注册账户，将申请的 API Token 填入下方或BoxJs
const aqicnToken = $.getdata($.token) || 'f85997883c9e5981248a2cd5ff40d97018df651e';

// STEP 2: 参考下方配置片段，在代理工具的配置文件中添加对应的配置。注意：script-path 后应该替换为添加 apicnToken 值后的脚本路径
/*
	[Script]
	AQI-US = type=http-response, requires-body=1, pattern=^https:\/\/weather-data\.apple\.com\/v1\/weather\/[\w-]+\/([0-9]+\.[0-9]+)\/([0-9]+\.[0-9]+)\?, script-path=https://raw.githubusercontent.com/DantoNGL/SS-Rule-Snippet/master/Scripts/Surge/iOS_Weather_AQI_Standard.js
...
function constructAirQuailityNode(aqicnData) {
	...
	airQualityNode.primaryPollutant = getPrimaryPollutant(
		aqicnData.dominentpol
	);
